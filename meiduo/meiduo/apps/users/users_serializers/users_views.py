...
from meiduo.utils.my_openid import decode_openid
...
from users.users_serializers import users_serializers
...
# 视图集实现用户地址操作
class UserAddressViewSet(ModelViewSet, MyLoginRequiredMixin):
    serializer_class = users_serializers.UserAddressSerializer
    queryset = Address.objects.filter(is_deleted=False).all()

    def get_queryset(self):
        addresses = Address.objects.filter(user=self.request.user, is_deleted=False).all()
        return addresses

    def list(self, request, *args, **kwargs):
        addresses = self.get_queryset()
        serializer = self.get_serializer(instance=addresses, many=True)
