...
from meiduo.utils.my_constants import USER_ORDER_PAGE_COUNT
...
# 展示订单评价页面
class OrderCommentView(MyLoginRequiredMixin):
    def get(self, request, order_id):
        ...
        try:
            ...
            return http.HttpResponseForbidden("订单不存在，请重试。")

        # check the order is comment or not
        if order_info.status != 4:
            return http.HttpResponseForbidden("该订单不能进行评价，请重试。")

        # 获取订单所有的商品
        skus = order_info.skus.all()
